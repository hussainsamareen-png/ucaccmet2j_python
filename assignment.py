...
while month <= 12:
    ...
    for measurement in seattle_data:
        if measurement["date"].startswith(f"2010-0{month}-"):
            ...
            total_monthly_precipitation[month] += measurement['value']
    month += 1
total_monthly_precipitation_list = list(total_monthly_precipitation)
...
relative_monthly_precipitation_list = list (relative_monthly_precipitation)
print(f'The relative monthly percipitation in seatle for each month is as follows {relative_monthly_precipitation_list}')
